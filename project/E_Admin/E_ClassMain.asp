        ...
        <tr>
          <td height="400" align="center" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0" background="Img/menu_12.jpg">
            <tr>
              ...
              <td class="Wryh T14 P2"><strong>&nbsp;分类管理</strong></td>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
             ...
             <table width="100%" border="0" cellpadding="0" cellspacing="0" background="Img/Banner.jpg">
               <tr>
                 ...
                 <td height="40" class="Wryh T13" width="100">下级小分类</td>
                 <td width="60" align="center" class="Wryh T13">状态</td>
...
Sql="select * from BClass order by ID Asc"
...
%>
               <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='<%If iRs mod 2 = 0 then response.Write("#d5ffff") Else response.Write("#f8ffd1") End if%>'" style="cursor:hand">
                 ...
                 <td><%=Rs("BClassName")%></td>
                 <td height="40" class="Wryh T13" width="100"><a href="E_SClassMain.asp?UpId=<%=Rs("Id")%>"><span class="Blue">管理小类&nbsp;(<%=Conn.execute("Select count(Id) from SClass where IfShow = 0 and UpId = "&Rs("Id"))(0)%>)</span></a></td>
                 <td width="60" align="center" class="Time"><%If Rs("IfShow") = 1 Then%><a href="E_ManageSave.asp?SaveType=IfShowEdit&IfShow=0&ID=<%=Rs("ID")%>"><img src="Img/web_icon_009.gif" alt="无效"></a><%Else%><a href="E_ManageSave.asp?SaveType=IfShowEdit&IfShow=1&ID=<%=Rs("ID")%>"><img src="Img/web_icon_006.gif" alt="有效"></a><%End if%></td>
                 <td width="80" align="center"><a href="E_BClassEdit.asp?ID=<%=Rs("ID")%>"><img src="Img/Edit.Gif" width="16" height="16"></a>&nbsp;&nbsp;<a href="#" onClick="ifmsgbox('将本条内容彻底删除吗？\n\n删除不可恢复，其相关的文件也将一并删除！','E_ManageSave.asp?SaveType=BClassDel&Id=<%=Rs("Id")%>&CurPage=<%=CurPage%>');"><img src="Img/Del.gif"  alt="删除"></a></td>
               </tr>
               <tr>
               ...
               <tr bgcolor="">
                 <td height="21"><div class="scott">分页：
                   <%if CurPage>dispage then%>
                         <a href="<%=filename%>?CurPage=1&SortID=<%=SortID%>&Key=<%=Key%>&MType=<%=MType%>"><font title="首页"><<</font></a>
                         <%end if%>
                         <%if CurPage>dispage then%>
                         <a href="<%=filename%>?CurPage=<%=StartPageNum-1%>&SortID=<%=SortID%>&Key=<%=Key%>&MType=<%=MType%>"><font title="上<%=dispage%>"><</font></a>
                         <%end if 
                  For I=StartPageNum to EndPageNum 
                  if I<>CurPage then %>
                         <a href="<%=filename%>?CurPage=<%=I%>&SortID=<%=SortID%>&Key=<%=Key%>&MType=<%=MType%>"><%=I%></a>
                         <% else %>
                         <span class="fya"><%=I%></span></font>
                         <% end if 
                  Next %>
                         <% if EndPageNum<RS.Pagecount then %>
                         <a href="<%=filename%>?CurPage=<%=EndPageNum+1%>&SortID=<%=SortID%>&Key=<%=Key%>&MType=<%=MType%>"><font title="下<%=dispage%>">></font></a>
                         <%end if 
                  if CurPage<TotalPages then%>
                         <a href="<%=filename%>?CurPage=<%=TotalPages%>&SortID=<%=SortID%>&Key=<%=Key%>&MType=<%=MType%>"><font title="尾页">>></font></a>
                         <%end if%>
                         <%end if%>
                 </div></td>
                 <td width="20">&nbsp;</td>
