...
<table width=400 border=0 align="center" cellpadding="12" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td width=100% height=100% align="center" bgcolor="#FFFFFF" class=tablebody1 >
<%
...
sub UpFile()
  ...
  if Upload.err > 0 then
    ...
    SaveToPath="../Upload/PicFiles" '文件保存目录,此目录必须为程序可读写
    if SaveToPath="" then
      SaveToPath="../"
    end if
    '在目录后加(/)
    ...
    for each FormName in Upload.file '列出所有上传了的文件
      ...
      if file.FileSize>0 then '如果 FileSize > 0 说明有文件数据
        ...
        if Result="ok" then
          ...
          response.write "<td nowrap><input type='text' size='56' class='input' value='"&right(FileName,len(FileName))&"'></td>"
          response.write "</tr>"
          response.write "<tr>"
          response.write "<td nowrap height='30'>文件大小：</td>"
          response.write "<td nowrap><input type='text' size='56' class='input' value='"&GainFileSize(file.Filesize)&"'></td>"
          response.write "</tr>"		  
          response.write "<tr>"
...
Private Function CheckFileExt (FileEXT)
  dim ForumUpload
  ForumUpload="exe,gif,jpg,jpeg,rar,zip,doc,wmv,wma,mp4,wav,docx,xlsx,xls,flv"
