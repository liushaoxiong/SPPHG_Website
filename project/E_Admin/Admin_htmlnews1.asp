...
totalrec=Conn.Execute("select count(*) from E_News Where ClassID = "&ClassID)(0)
sql="Select * from E_News  Where ClassID = "&ClassID&" order by ID desc"
...
do while not rs.eof
ID=rs("ID")
call htmll("","","/News/E_NewsShow-"&ID&"-"&Rs("ClassID")&"-"&encode(ID)&".html","Web/NewsShow.asp","ID=",ID,"","")
